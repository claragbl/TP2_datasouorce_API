...
class TestEndpointRoutes:
    ...
    def test_get_firestore_collection_parameters(self, client):
        """
        Test the /data/get_firestore_collection_parameters of the API that return the parameters of the Firestore collection in a dictionary.

        Returns:
            None
        """
        response = client.get("/data/get_firestore_collection_parameters")
        assert response.status_code == 200
        assert response.headers["content-type"] == "application/json"
        data = response.json()
    ...
    def test_update_firestore_collection_parameters(self, client):
        """
        Test the /data/update_firestore_collection_parameters endpoint of the API that update the parameters of the Firestore collection in the database.
        
        Returns:
            None
        """
        new_value = 200
        response = client.get(f"/data/update_firestore_collection_parameters?n_estimators={new_value}")
        data = response.json()

        assert response.status_code == 200
        assert "n_estimators" in data
        assert data["n_estimators"] == new_value
